 ...
 *
 *
 * We use the code of the author Henrik Lehman as reference
 *
 *
 *-----------------------------------------------------*/
#include <stdio.h>
...
int main(int argc, char **argv) {
    float tempo[] = {0,0,0,0,0};
        int *loc_mat, *loc_dist, *loc_pred, *global_dist = NULL, *global_pred = NULL;
        int my_rank, p, loc_n, n;
        int time1, time2,totalTime;
        MPI_Comm comm; //The communicator that will determine which proccess are involved in a communication
        ...
        time1 = MPI_Wtime();
        comm = MPI_COMM_WORLD;//Crea el comunicador que sera la 'caja' en donde se comunicaran todos los procesos
        MPI_Comm_rank(comm, &my_rank); //Esta función devuelve el rango del proceso que se almacena en &my_rank.
        MPI_Comm_size(comm, &p); //Determina el tamaño del comunicador seleccionado, es decir, 
								 //el número de procesos que están actualmente asociados a este guardandolo en &p.
        n = Read_n(my_rank, comm);
        loc_n = n / p; //N debe ser divisible entre p
        ...
        MPI_Finalize();
        printf("Total Time%d\n",totalTime);
        writeInFile("home/u13/times.txt", tempos, 5);
    
    return 0;
}

void writeInFile(char path[] , float data[], int n){
    ...
    for(int i = 0; i < n;i++){
        //Used for write ProcessorsFile
        fprintf (p, "%f\n",data[i]);
        //Used for write the dataFile
        //fprintf (p, "%d %f\n",(i+1)*50,data[i]);
    }
